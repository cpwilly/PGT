...
async function getData(DataType, tag) {
    try{
        ...
        if(DataType == DataType.Dorm){
            ...
            models = await DataStore.query(Account, (c) => c.email.eq(tag));
        }
        return models;
    } catch (Error){
        console.log(Error);
    ...
    try{
        ...
        case 1:
            await DataStore.save(Dorm.copyOf(CURRENT_ITEM, item => {
                ones: CURRENT_ITEM.ones + 1;
            }));
        case 2:
            await DataStore.save(Dorm.copyOf(CURRENT_ITEM, item => {
                twos: CURRENT_ITEM.twos + 1;
            }));
        case 3:
            await DataStore.save(Dorm.copyOf(CURRENT_ITEM, item => {
                threes: CURRENT_ITEM.threes + 1;
            }));
        case 4:
            await DataStore.save(Dorm.copyOf(CURRENT_ITEM, item => {
                fours: CURRENT_ITEM.fours + 1;
            }));
        case 5:
            await DataStore.save(Dorm.copyOf(CURRENT_ITEM, item => {
                fives: CURRENT_ITEM.fives + 1;
            }));
        default:
    }
    } catch (Error){
